...
export default function ResultForm({ existing, onSuccess, onClose }: Props) {
  ...
  const [studentId,  setStudentId]  = useState(existing?.student_id?.toString() ?? "");
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/students`, {
      ...
      .then((r) => r.json())
      .then((data) => {
        console.log("Students response:", data); // ← see what's coming back
        setStudents(Array.isArray(data) ? data : []); // ← safe
      })
      .catch(() => setStudents([]));
  }, []);
  ...
  const subjects = [
    ...
    "Islamic Studies", "Fiqh", "Aqeedah", "Hadith", "Seerah",
    "Islamic Manners",
  ];
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box animate-scale-in">
        <div className="modal-header">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-forest-400 hover:bg-forest-50 transition-colors">
            <X size={18} />
          </button>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="form-group mb-0">
                ...
                <div className={`form-input flex items-center font-bold text-lg ${
                  grade === "A+" || grade === "A" ? "text-forest-600" :
                  grade === "B"                   ? "text-gold-600"   :
            ...
            <div className="modal-footer px-0 pb-0 pt-2">
              ...
              <button type="submit" disabled={status === "loading"} className="btn btn-secondary btn-sm">
                {status === "loading" ? "Saving…" : isEdit ? "Save Changes" : "Upload Result"}
              </button>
            </div>
          </form>
        </div>
